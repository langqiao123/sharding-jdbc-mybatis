...
public interface TUserDoMapper {
     ...
     * 通用查询行数方法，可设置查询属性
     */
    Long selectCountSelective(@Param("record") TUserDo record);

    /**
     * 通用查询List方法，可设置查询属性，排序字段，分页参数 
     */
    List<TUserDo> selectPageSelective(@Param("record") TUserDo record, @Param("pageable") Pageable pageable);
     ...
     * 插入行记录返回自增ID
     */
    Integer insertSelectiveReturnPrimaryKey(TUserDo record);

    /**
     * 批量插入行数据返回插入行数
     */
    Integer batchInsert(List<TUserDo> records);
